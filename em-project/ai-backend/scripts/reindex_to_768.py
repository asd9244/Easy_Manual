...
# 모델 초기화 (로컬에서 썼던 것과 동일한 1024차원 BGE-M3 모델 사용)
# 처음 실행 시 모델 다운로드로 인해 시간이 약간 소요될 수 있습니다.
embeddings_model = SentenceTransformer('BAAI/bge-m3')

def reindex():
    print("🚀 [Step 1] 로컬 엔진(BGE-M3)을 이용한 1024차원 재임베딩 작업 시작...")
    ...
    with GraphDatabase.driver(NEO4J_URI, auth=(NEO4J_USER, NEO4J_PASSWORD)) as driver:
        with driver.session() as session:
            ...
            for idx, record in enumerate(sections):
                ...
                # 2. 로컬 엔진 임베딩 (1024차원) 생성
                try:
                    new_embedding = embeddings_model.encode(text).tolist()

...
import {WashingMachine, ChevronDown, Settings} from "lucide-react";
    ...
    <GlassCard className="active:scale-[0.98] group cursor-pointer">
      ...
      >
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div
            className={`px-3 py-1.5 rounded-full text-[11px] font-bold flex items-center gap-1.5 ${badgeStyle}`}
          >
            <div
              className={`w-1.5 h-1.5 rounded-full ${status === "정상" ? "bg-theme-primary" : status === "점검 필요" ? "bg-yellow-500" : "bg-red-500"}`}
            />
            {status}
          </div>
          {/* 💡 열리면 화살표가 위로 휙! 돌아감 */}
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-4 mb-6 text-left">
              ...
              <div className={isWarning ? "bg-red-50 -m-2 p-2 rounded-xl" : ""}>
                <p
                  className={`text-[10px] font-bold flex items-center gap-1 ${isWarning ? "text-red-400" : "text-slate-400"}`}
                >
                  <span
                    className={isWarning ? "text-red-300" : "text-slate-300"}
                  >
                    ⚗️
                  </span>{" "}
                  필터 교체
                ...
                <p
                  className={`text-sm font-bold mt-1 ${isWarning ? "text-red-500" : "text-slate-700"}`}
                >
                  {filterStatus}
                </p>
              </div>
              <div>
                <p className="text-[10px] text-slate-400 font-bold flex items-center gap-1">
                  <span className="text-slate-300">🔧</span> 수리 이력
                </p>
                <p className="text-sm font-bold text-slate-700 mt-1">
                  {repairCount}
                </p>
